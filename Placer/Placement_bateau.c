...
int Commencer_jeu_placement_bateau(void){ /*Fonction qui initialise completement la grille et qui demande à l'utilisateur de placé les bateaux*/
	...
	curs_set( 0 ); // hide the default screen cursor.
       ...
       printw("\n\nChoix des bateaux : ");
       refresh();
	while(((ch=wgetch(w)) != 10 && strcmp(list[i],"Choisir son nombre de bateaux : 2")==0) || ((ch!= 10 && (strcmp(list[i],"Un seul bateau de même type : 1")==0)))){ //Tant qu'on a pas appuyé sur entrer sur l'un des deux items
		...
		mvwprintw( w, i+1, 2, "%s", item);
		wattroff( w, A_STANDOUT );
		if(ch == 10 && strcmp(list[i],"Un seul bateau de même type : 1")==0){
			eConfig=1;
			wrefresh(w); // Refresh it (to leave it blank)
			delwin(w); // and delete
            	}else if(ch == 10 && strcmp(list[i],"Choisir son nombre de bateaux : 2")==0){
			eConfig=2;
			wrefresh(w); // Refresh it (to leave it blank)
			delwin(w); // and delete
            	}
