...
void Enlever_grillebateau(t_bateau bateau, int i,int j,int num_grille,int choix_sens){
	...
	if(choix_sens==1){
		while(compteur<bateau){
			...
			if(bat==0 && obs==0)Grille_ecrire_bateau(i,j+compteur,num_grille,Aucun_b);
			compteur++;
		}
	}else if(choix_sens==2){
		while(compteur<bateau){
			...
			if(bat==0 && obs==0)Grille_ecrire_bateau(i+compteur,j,num_grille,Aucun_b);
			
			compteur++;
		}
	}
}
void Enlever_grillebateau2(){
	int i,j;
	for(i=0;i<N;i++){
		for(j=0;j<M;j++){
			if(grille[i][j].bateau==Sous_Marin || grille[i][j].bateau==Destroyer || grille[i][j].bateau==Porte_Avion || grille[i][j].torpilleur==Present){
...
void Placer_bateau_auto(int num_grille,int nb_torpilleur){
	...
	printf("Etes vous satisfait de vos bateaux ?? Oui:1 Non:0");
			...
			valide=0;
Enlever_grillebateau2();


		}
            }else  Enlever_grillebateau2();
	}while(!valide);
}
